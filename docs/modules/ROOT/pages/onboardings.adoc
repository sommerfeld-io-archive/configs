...
. Setup Dependabot by configuring the repo (_Settings -> Code security and analysis -> Dependabot security updates_) and running the `github-actions` module from `jarvis`, then adjust (if necessary):
.. The build triggers of `auto-generate-docs.yml` and `ci.yml`
.. Add `if: ${{ github.actor != 'dependabot[bot]' }}` to Docker Login, the push setting from Docker build and both Snyk steps
